...
Click **Test Recall & ngrok** in Caddy. **Synthetic checks passed** means the four synthetic components passed; it creates no meeting bot and does not verify real Recall transcript delivery, dashboard event selections, that the entered signing secret matches the workspace, or provider retention. A red failure automatically opens **Diagnostic details**; follow the first blocking component. Authentication rejected means check the API key and US West workspace, while Recall unavailable means authentication could not be verified and calls for retry/network/service checks—not a claim that the key is wrong. Recheck credentials only when their own component failed. Do not stop an unfamiliar process. Use **Copy the secret-free diagnostic summary for your agent** to troubleshoot without sharing field values.
...
The check sends a signed synthetic request from this Mac through a temporary public tunnel back to Caddy. Locally signing and checking with the same entered secret does not prove that secret matches Recall’s workspace secret. It does not prove that Recall can deliver a real webhook, that dashboard event subscriptions are correct, or that provider retention matches expectations. Caddy closes the tunnel after the test, so a later browser or command-line HTTP “offline” response is different from a failure while the app check was running. A custom callback domain is not a proven fix and is not required by this guidance. The check remains advisory: saving complete valid settings is separate from its outcome, while actual capture startup safety remains enforced.
...
If Connection Settings shows older wording and no **Copy the secret-free diagnostic summary for your agent** action after an error, that identifies an older running Caddy copy. Preserve its draft; do not reset credentials or delete app data. Use that installed copy’s guidance until a separately approved matching update is available.
...
Interpret **Test Recall & ngrok** through [Public callback recovery](#public-callback-recovery). An amber warning means this Mac’s synthetic public route is unverified, not that Recall delivery is known to fail. Point the human to the [practice interview](docs/practice-interview.md); only the human starts capture once, admits the visible bot, speaks the synthetic sentence, and confirms live transcript text, bot departure, and Caddy finalization. An agent must not perform or automate those live actions.
