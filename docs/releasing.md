...
G1 (Apple access/signing/notary), G2 (destination/version), G3 (downloaded real-machine and human/agent acceptance), and G4 (publication) remain required. A locally ad-hoc artifact is never the public deliverable.
...
`Convo Caddy-0.1.0-arm64.dmg` and
`Convo Caddy-darwin-arm64-0.1.0.zip`. Their hashes are generated anew by
