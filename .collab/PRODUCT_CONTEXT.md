...
Setup errors use concise human status plus one text action to copy a current, bounded, secret-free diagnostic summary for an agent. This applies to Recall/ngrok checks, Hermes discovery and assistant tests, Save, Reset, and setup loading. Copied summaries contain only allowlisted states, codes, statuses, and fixed guidance—never raw upstream errors, credentials, callback URLs, transcript content, filesystem/user/host/SSH identifiers, or field values. Editing or rerunning invalidates stale summaries. Deeper setup explanation belongs in the Markdown guide and README, not repeated inline. Save remains the primary form action; Reset is a spatially separated, lower-emphasis destructive action rather than a peer primary control.
